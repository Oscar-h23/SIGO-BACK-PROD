...
public class TrabajadorController {
    ...
    public List<TrabajadorResponse> listarAgentesPorPlaza(@RequestParam Long plazaId) {
        return r.findAgentesByPlaza(plazaId).stream()
                .map(TrabajadorResponse::from)
                .toList();
    }

    @GetMapping("/controladores")
    public List<TrabajadorResponse> listarControladoresPorPlaza(@RequestParam Long plazaId) {
        return r.findControladoresByPlaza(plazaId).stream()
                .map(TrabajadorResponse::from)
                .toList();
    }
}
