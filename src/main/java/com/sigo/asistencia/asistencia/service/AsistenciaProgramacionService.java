...
public class AsistenciaProgramacionService {
    ...
    public int obtenerProgramados(Long plazaId, Long turnoId) {
        ...
        String plazaCodigo = plaza.getCodigo() == null ? "" : plaza.getCodigo().trim().toUpperCase(Locale.ROOT);
        String turnoCodigo = turno.getCodigo() == null ? "" : turno.getCodigo().trim().toUpperCase(Locale.ROOT);

        return switch (plazaCodigo) {
            case "P1" -> porTurno(turnoCodigo, 17, 17, 9);
            case "P2", "P3" -> porTurno(turnoCodigo, 25, 25, 11);
            case "P4", "P5" -> porTurno(turnoCodigo, 7, 7, 3);
            case "P6", "P7" -> porTurno(turnoCodigo, 13, 12, 5);
