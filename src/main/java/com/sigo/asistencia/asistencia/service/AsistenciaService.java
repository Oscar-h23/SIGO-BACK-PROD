...
public class AsistenciaService {
    ...
    public List<AsistenciaResponse> listar(
        ...
        /*
         * Antes: por cada registro se llamaba a obtenerPorId(id),
         * que ejecutaba 3 queries adicionales (findById, ausencias, evidencias).
         * Para N registros eso era 1 + 3N queries (N+1).
         *
         * Ahora: solo 2 queries para TODAS las ausencias y evidencias del rango,
         * y además se ejecutan EN PARALELO (son independientes entre sí), para no
         * pagar dos veces la latencia fija de ida y vuelta a la base de datos.
         */
        java.util.concurrent.CompletableFuture<Map<Long, List<AusenciaResponse>>> ausenciasFuture =
                        ...
                        ausenciaRepository
                                ...
                                .collect(
                                        Collectors.groupingBy(
                                                ausencia -> ausencia.getAsistencia().getId(),
                                                Collectors.mapping(
                                                        ausencia -> new AusenciaResponse(
                                                                ausencia.getId(),
        ...
        java.util.concurrent.CompletableFuture<Map<Long, List<EvidenciaResponse>>> evidenciasFuture =
                java.util.concurrent.CompletableFuture.supplyAsync(() ->
                        evidenciaRepository
                                .findByAsistenciaIdIn(ids)
                                .stream()
                                .collect(
                                        Collectors.groupingBy(
                                                evidencia -> evidencia.getAsistencia().getId(),
                                                Collectors.mapping(
                                                        evidencia -> new EvidenciaResponse(
                                                                evidencia.getId(),
                                                                evidencia.getUrlArchivo(),
                                                                evidencia.getTipo()
                                                        ),
        ...
        Map<Long, List<AusenciaResponse>> ausenciasPorAsistencia = ausenciasFuture.join();
        Map<Long, List<EvidenciaResponse>> evidenciasPorAsistencia = evidenciasFuture.join();
        return registros
                .stream()
                .map(registro -> new AsistenciaResponse(
                        registro.getId(),
                        ...
                        registro.getProgramados() - registro.getPresentes(),
                        registro.getApoyoSolicitado(),
                        registro.getDetalleApoyo(),
                        registro.getPorcentaje(),
                        registro.getNotas(),
                        ausenciasPorAsistencia.getOrDefault(registro.getId(), List.of()),
                        evidenciasPorAsistencia.getOrDefault(registro.getId(), List.of())
                ))
                .toList();
    }
